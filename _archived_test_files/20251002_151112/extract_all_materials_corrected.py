"""
CORRECTED Material Extraction Script for EliteMining
Extracts all 13 materials with proper column mapping and manual confirmation
"""
...
def get_material_urls(material_name):
    """Generate correct 43 URLs for a material across 9 directional sectors"""
    urls = [
...
def extract_material(material_name):
    """Extract hotspot data for a single material"""
    print(f"\n{'='*60}")
    print(f"EXTRACTING: {material_name}")
    print(f"{'='*60}")
    
    urls = get_material_urls(material_name)
    ...
    print(f"\nRaw {material_name} records: {len(all_data)}")
    ...
    # Save to file
    output_file = f'app/data/Hotspots/{material_name}_extracted_corrected.xlsx'
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    df_cleaned.to_excel(output_file, index=False)
    
    print(f"Cleaned {material_name} records: {len(df_cleaned)}")
...
def main():
    """Main extraction process with manual confirmation"""
    
    # Remaining 11 materials to extract (Platinum and Alexandrite already done)
    materials = [
        'Benitoite',
        'Bromellite',
        'Grandidierite',
    ...
    print("CORRECTED MATERIAL EXTRACTION SCRIPT")
    print("=====================================")
    print("This script will extract remaining 11 materials with:")
    print("- CORRECT column mapping (LS from column 5)")
    print("- PROPER ring name cleaning (preserves body identifiers)")
    print("- NO manual confirmation (auto-continue)")
    print(f"- Processing {len(materials)} remaining materials")
    print("- Platinum and Alexandrite already completed")
    ...
    for i, material in enumerate(materials, 1):
        print(f"\n\nMATERIAL {i}/{len(materials)}: {material}")
        
        try:
            record_count = extract_material(material)
            results[material] = record_count
            
            print(f"\n✅ {material} extraction completed: {record_count} records")
            
            # No manual confirmation needed for remaining materials
            if i < len(materials):
                print(f"Proceeding to next material: {materials[i]}")
                time.sleep(3)  # Brief pause between materials
            
        except Exception as e:
            print(f"\n❌ ERROR extracting {material}: {e}")
            results[material] = 0
            
            # Auto-continue on errors for remaining materials
            print("Continuing to next material...")
            time.sleep(2)
    ...
    total_records = 0
    for material, count in results.items():
        status = "✅" if count > 0 else "❌"
        print(f"{status} {material:20s}: {count:,} records")
        total_records += count
    
    print(f"\nTotal records extracted: {total_records:,}")
    print("\nAll files saved to: app/data/Hotspots/")
    print("Next step: Run cleanup and combination process")
